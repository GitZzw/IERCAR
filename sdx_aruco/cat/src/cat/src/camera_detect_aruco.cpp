...
void arucoDetect::initPubSub(ros::NodeHandle &nh){
    ...
    sub = nh.subscribe("/camera/color/image_raw", 100,&arucoDetect::imageCb, this);
}


//zzw_added for t265 fisheye
    ...
    while (systemOk()) {
        ...
        if (!ids.empty()) {
            ...
            if(find(ids.begin(), ids.end(), 4)!=ids.end()){
                ...
                cv::aruco::estimatePoseSingleMarkers(corner, 0.08, cameraMatrix, distCoeffs, rvecs,
                                                     tvecs);
                //transform
            ...
            cv::aruco::estimatePoseSingleMarkers(corners, 0.08, cameraMatrix, distCoeffs, rvecs,
                                                 tvecs);
            ...
            if (display) {
                cv::aruco::drawDetectedMarkers(imageCopy, corners, ids, Scalar(0, 0, 255));
